...
def generate_random_array(n):
    if array_mode == ArrayType.PARTIALLY_SORTED:
        ...
        sortable_arr = generate_partially_sorted_array(n, int(n*degree_of_sortedness))
...
button_bubble_average = tk.Button(root, text="100x bubblesort average", command=lambda: average_sort(bubble_sort))
button_quick = tk.Button(root, text="single quicksort", command=lambda: sort_once(quicksort))
button_bubble_quick = tk.Button(root, text="100x quicksort average", command=lambda: average_sort(quicksort))
button_merge = tk.Button(root, text="single merge sort", command=lambda: sort_once(merge_sort))
button_merge_average = tk.Button(root, text="100x merge sort average", command=lambda: average_sort(merge_sort))
